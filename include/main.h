...
// print debug output periodically
...
#define ENCODER_STATUS_REG    0x0000
#define ENCODER_ANGLE_REG     0x0020
#define ENCODER_SPEED_REG     0x0030
#define ENCODER_ANGLE_REV_REG 0x0040
#define ENCODER_TEMP_REG      0x0050
#define KEEP_15_BITS          0x7FFF
#define KEEP_8_BITS           0x00FF
#define POW_2_15              32768.0f
#define TEMP_OFFSET           152.0f
#define TEMP_DIV              2.776f
// motor current constants
#define POWER_2(N) (1U << N)
#define BOARD_VOLTAGE 3.3f
#define CURRENT_SENSE_RESISTOR 0.2f
#define PWM_COMPARE_FORMAT RESOLUTION_9B_COMPARE_FORMAT
#define PWM_MAX_VALUE (POWER_2(PWM_COMPARE_FORMAT) - 1)
// safety limits
#define MAX_TEMPERATURE 50   // maximum sensor temperature
#define STALL_MIN_ANGLE 2.5f // minimum angle the motor must move between STALL_INTERVAL checks
// timings
#define STALL_INTERVAL 500                  // interval to check if the motor has stalled (ms)
#define ANGLE_INTERVAL 3200                 // time between angle reads in microseconds
#define STEP_INTERVAL  (51200 / MICROSTEPS) // time between microsteps

enum class ControlStatus {
	NO_ERROR               = 0x00,  // all good
	SYSTEM_ERROR           = 0x01,  // sensor system error
	INTERFACE_ACCESS_ERROR = 0x02,  // sensor access error
	INVALID_ANGLE_ERROR    = 0x03,  // sensor angle reading invalid
    STALL_ERROR            = 0x04,  // motor stall check failed
	CRC_ERROR              = 0x05,  // sensor crc error
    OVERTEMP_ERROR         = 0x06,  // over temperature protection
};

// instances
SPI_HandleTypeDef mSPIConfig;
GPIO_InitTypeDef GPIO_InitStructure;
HardwareTimer mPWMTimer((TIM_TypeDef *)pinmap_peripheral(COIL_A_POWER_OUTPUT_PIN, PinMap_TIM));
uint32_t mChannelA = STM_PIN_CHANNEL(pinmap_function(COIL_A_POWER_OUTPUT_PIN, PinMap_TIM));
uint32_t mChannelB = STM_PIN_CHANNEL(pinmap_function(COIL_B_POWER_OUTPUT_PIN, PinMap_TIM));

// variables
bool mDriving = false;
int32_t mMotorPosition = 0;
uint32_t mLastStep = 0, mLastSensorRead = 0, mLastStallCheck = 0;
float mOutputAngle = 0.0f, mTargetAngle = -1.0f, mDeviation = 0.0f, mSensorTemp = 0.0f;
int16_t mSlackCompensation = 0;
ControlStatus mStatus = ControlStatus::NO_ERROR;
#if DEBUG_PRINT
    uint32_t mLastPrint = 0;
#endif
...
void TLE5012B_init();
void TLE5012B_read(uint16_t registerAddress, uint16_t* data);
uint8_t TLE5012B_CRC(uint8_t *message, uint8_t length);

// sine lookup table, amplitude 2^10, count 2^12
const float mSineLookup[] = {
